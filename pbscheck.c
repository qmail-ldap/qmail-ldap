...
void die_usage() { err("554 usage: smtpallow subprogram (#5.3.5)"); die(); }
void die_exec() { err("554 unable to start smtp daemon (#5.3.5)"); die(); }
void die_badenv() { err("554 unable to read $TCPREMOTEIP (#5.3.5)"); die(); }
void die_control()
{
  err("421 unable to read controls (#4.3.0)");
  log("smtpallow unable to read controls");
  die();
}
void die_nomem()
{
  err("421 out of memory (#4.3.0)");
  log("smtpallow out of memory");
  die();
}
...
int main (int argc, char** argv)
{
  ...
  *s++ = 0; len++;   /* status */
  ...
  do {
    ...
    if(t >= numservers) {
      log("smtpallow: no response from server");
